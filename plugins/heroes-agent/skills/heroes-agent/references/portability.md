...
- Credential configuration UX differs, but all runtime scripts read the same ignored `self/.env` or `.env` names. Cursor manifest variables are not used because they would not populate this local runtime automatically.
