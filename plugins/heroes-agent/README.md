...
`sync-rate-catalog.ts` is read-only and uses the existing peer credential. `ingest-rates.ts --dry-run` still needs an existing inbox. A CSV with no rate rows remains in the inbox and does not create an index. `find-rate.ts` exits `2` until both `heroes-catalog.json` and `rate-catalog.json` exist. It exits `3` when no automatic result is available, including missing facts or ambiguity. It exits `4` for an invalid query or catalog. It never selects the cheapest result.
